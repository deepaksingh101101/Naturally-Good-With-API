...
import { getCityById } from '@/app/redux/actions/cityActions';
...
export default function Page() {
  ...
  const citys:any = useSelector((state: RootState) => state.citys.selectedCity);
  const loading = useSelector((state: RootState) => state.citys.loading); // Assuming you have a loading state
  const isDisabled=false;
  useEffect(() => {
    // Check if the ID is present before dispatching the action
    if (id) {
      dispatch(getCityById(id as string)); // Dispatch the action to fetch employee by ID
    }
  }, [id, dispatch]);

  return (
    <MainLayout meta={{ title: 'City Management' }}>
      <ScrollArea className="h-full">
        <div className="flex-1 min-h-screen space-y-4 p-4 pt-6 md:p-8">
          ...
          {loading ? ( // Show loading state if data is being fetched
            ...
            <CityForm initialData={citys} isDisabled={isDisabled} />
          )}
        </div>
      </ScrollArea>
    </MainLayout>
