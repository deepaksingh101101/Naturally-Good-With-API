...
import { getComplainTypeById } from '@/app/redux/actions/complaintActions';
...
export default function Page() {
  ...
  const complainType:any = useSelector((state: RootState) => state.complainType.selectedComplaint);
  const loading = useSelector((state: RootState) => state.complainType.loading); // Assuming you have a loading state
  const isDisabled=false;
  useEffect(() => {
    // Check if the ID is present before dispatching the action
    if (id) {
      dispatch(getComplainTypeById(id as string)); // Dispatch the action to fetch employee by ID
    }
  }, [id, dispatch]);
      ...
      <ScrollArea className="h-full">
        <div className="flex-1 min-h-screen space-y-4 p-4 pt-6 md:p-8">
          ...
          {loading ? ( // Show loading state if data is being fetched
            <p>Loading complain Type data...</p>
          ) : (
            <ComplaintForm initialData={complainType} isDisabled={false} />
          )}
        </div>
      </ScrollArea>
    </MainLayout>
